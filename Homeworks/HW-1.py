...
sentWords=sentences.map(lambda x: cleanText(x)).cache()
...
for n in range(1,6):
    ...
    ngramRDD = sentWords.flatMap(lambda x: [(tuple(x[i:i+n]), 1) for i in xrange(len(x)-n+1)]).reduceByKey(lambda a,b: a+b).map(lambda (k,v): (v,k))
    freq_ngramRDD = ngramRDD.sortByKey(False)
